...
module OpenApiSDK
  ...
  class Artifacts
    ...
    sig { params(namespace_name: ::String).returns(::OpenApiSDK::Operations::ListRemoteSourcesResponse) }
    def list_remote_sources(namespace_name)
      # list_remote_sources - Get remote sources attached to a particular namespace
      request = ::OpenApiSDK::Operations::ListRemoteSourcesRequest.new(
      ...
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::ListRemoteSourcesRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['user-agent'] = @sdk_configuration.user_agent
      ...
      res = ::OpenApiSDK::Operations::ListRemoteSourcesResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status >= 200 && r.status < 300
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Shared::RemoteSource)
          res.remote_source = out
        end
      elsif r.status >= 400 && r.status < 500
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Shared::Error)
    ...
    sig { params(request: T.nilable(::OpenApiSDK::Shared::RemoteSource)).returns(::OpenApiSDK::Operations::CreateRemoteSourceResponse) }
    def create_remote_source(request)
      ...
      res = ::OpenApiSDK::Operations::CreateRemoteSourceResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status >= 200 && r.status < 300
      elsif r.status >= 400 && r.status < 500
